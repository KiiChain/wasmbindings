...
#[entry_point]
pub fn instantiate(
    deps: DepsMut<KiichainQuery>,
    _env: Env,
    info: MessageInfo,
    _msg: InstantiateMsg,
) -> StdResult<Response<KiichainMsg>> {
...
pub fn execute(
    ...
    info: MessageInfo,
    msg: ExecuteMsg,
) -> Result<Response<KiichainMsg>, ReflectError> {
    match msg {
...
#[entry_point]
pub fn query(deps: Deps<KiichainQuery>, _env: Env, msg: QueryMsg) -> StdResult<QueryResponse> {
    match msg {
...
fn query_owner(deps: Deps<KiichainQuery>) -> StdResult<OwnerResponse> {
...
fn query_subcall(deps: Deps<KiichainQuery>, id: u64) -> StdResult<Reply> {
    let key = id.to_be_bytes();
    replies_read(deps.storage).load(&key)
}

fn query_chain(
    deps: Deps<KiichainQuery>,
...
mod tests {
    ...
    ) -> OwnedDeps<MockStorage, MockApi, MockQuerier<KiichainQuery>, KiichainQuery> {
        let custom_querier: MockQuerier<KiichainQuery> =
